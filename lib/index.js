...

{
  ...
  'apply': 'permit-overrides', // Relative to rules
  'rules': []
}
...

{
  ...
  'apply': 'permit-overrides', // Relative to policies
  'policies': []
}
...
internals.evaluatePolicy = function(item, information, callback) {
  ...
  if(!internals.combineAlg[item.apply]) {
    return callback(Boom.badImplementation('RBAC error: combination algorithm does not exist: ' + item.apply));
  }
...
internals.combineAlg['deny-overrides'] = function(items, information, fn, callback) {
  ...
  for(var i = 0; i <  items.length; i++) {
    tasks.push(fn.bind(items[i], information));
  }
