...
exports.register = (server, options, next) => {
    ...
    server.ext('onPostAuth', (request, reply) => {
        ...
        if (request.route.settings.plugins.rbac) {
            ...
            return internals.retrievePolicy(request, (err, policy) => {

                if (err) {
                    return reply(err);
                }

                internals.evaluatePolicy(request.route.settings.plugins.rbac, request.auth.credentials, (err, result) => {
 ...
 **/
internals.retrievePolicy = (request, callback) => {

    if (request.route.settings.plugins.rbac instanceof Function) {
        request.route.settings.plugins.rbac(reqeust, callback);
    }
    else {

        callback(null, request.route.settings.plugins.rbac);
    }
};
