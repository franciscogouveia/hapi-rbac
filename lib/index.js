 ...
 **/
schemas.rdr_handles = Joi.alternatives().try(
    Joi.string().min(1),
    Joi.array().items(Joi.string().min(1))
);
schemas.rdr_retriever = Joi.func().arity(3);
schemas.rdr_options = Joi.object({
    override: Joi.boolean().optional()
}).unknown(false);

defaults.rdr_options = {
    override: false
};

internals.registerDataRetriever = (handles, retriever, options) => {
    Joi.assert(handles, schemas.rdr_handles);
    Joi.assert(retriever, schemas.rdr_retriever);
    options = options || {};
    Joi.assert(options, schemas.rdr_options);
    options = Hoek.applyToDefaults(defaults.rdr_options, options);

    if(handles instanceof Array) {
        handles.forEach((source) => internals.registerSingleDataRetriever(source, retriever, options));
        return;
    } else {
        internals.registerSingleDataRetriever(handles, retriever, options);
    }
};

internals.registerSingleDataRetriever = (handles, retriever, options) => {
    if(internals.dataRetrievers[handles] && !options.override) {
        throw new Error('There is a data retriever already registered for the source: ' + handles);
    }

    internals.dataRetrievers[handles] = retriever;
};
 ...
 **/
schemas._rd_source = Joi.string().min(1);
schemas._rd_key = Joi.string().min(1);

internals.retrieveData = (source, key, context) => {
    Joi.assert(source, schemas._rd_source);
    Joi.assert(key, schemas._rd_key);

    if(!internals.dataRetrievers[source]) {
        return null;
    }

    return internals.dataRetrievers[source](source, key, context);
};

// Default data retrievers

defaults.credentialsRetriever = (source, key, request) => {
...
schemas.register_options = Joi.object({
    ...
    dataRetrievers: Joi.array().items(
        Joi.object({
            handles: schemas.rdr_handles.required()
        })
    ).optional()
});
...
exports.register = (server, options, next) => {
    ...
    internals.registerDataRetriever('credentials', defaults.credentialsRetriever, {override: true});
    ...
    server.ext('onPostAuth', (request, reply) => {
        ...
        if (config && config !== CONFIG_NONE) {
            ...
            return internals.retrievePolicy(config, request, (err, policy) => {
                ...
                internals.evaluatePolicy(policy, request.auth.credentials, (err, result) => {
...
internals.evaluateTarget = (target, information, callback) => {
    ...
    for (let i = 1; i < target.length; ++i) {
        const result = internals._targetApplies(target[i].value, information[target[i].type]);
...
if (['test', 'development'].indexOf(process.env.NODE_ENV) !== -1) {
    ...
    exports.registerDataRetriever = internals.registerDataRetriever;
    exports.retrieveData = internals.retrieveData;
}
